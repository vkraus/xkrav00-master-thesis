...
% Load inputenc early for pdflatex so it is available before biblatex
\ifthenelse{\boolean{xetex}\OR\boolean{luatex}}{}{\usepackage[utf8]{inputenc}}

%%% Nastavení pro použití samostatné bibliografické databáze.
%%% Settings for using a separate bibliographic database.
\input biblatex-setup

%% Set up fonts and encoding for English
...
%%% Fira Sans + Inconsolata (condensed mono)
\usepackage[sfdefault]{FiraSans}
\usepackage[narrow,var0,varqu,varl]{inconsolata}
\ifluatex
\makeatletter
...
\usepackage{microtype}      % font expansion and character protrusion — reduces overfull hboxes
\emergencystretch=1em       % extra stretch allowance to prevent overfull hboxes with \texttt
...
\renewcommand{\glsnamefont}[1]{\textbf{#1}}  % Bold acronyms
\setlength{\glsdescwidth}{0.8\textwidth}     % Left-aligned descriptions
% All acronym occurrences rendered in black (hyperlink still present, just uncoloured)
\definecolor{glsfirstuse}{RGB}{0,105,92}
...
% Fix list of figures/tables/listings spacing: neutralize parskip and chapter-group gaps
\makeatletter
\let\origlistoffigures\listoffigures
\renewcommand{\listoffigures}{\begingroup\parskip=0pt\let\addvspace\@gobble\origlistoffigures\endgroup}
\let\origlistoftables\listoftables
\renewcommand{\listoftables}{\begingroup\parskip=0pt\let\addvspace\@gobble\origlistoftables\endgroup}
\let\origlstlistoflistings\lstlistoflistings
\renewcommand{\lstlistoflistings}{\begingroup\parskip=0pt\origlstlistoflistings\endgroup}
\makeatother
...
\usepackage[normalem]{ulem}  % restore \underbar after sectsty redefines it

% \paragraph as block heading (own line), not run-in.
% The \@startsection after-skip must be positive; negative selects run-in.
\makeatletter
...
% \subsubsubsection: numbered fourth-level heading, same visual weight as the
% \paragraph redefine above. Shares LaTeX sectioning slot 4; secnumdepth=4
% below makes subsubsection and subsubsubsection both numbered. tocdepth
% stays at 2 (set in thesis.tex), so level-4 entries are written to the .toc
% file but filtered from the rendered table of contents.
\makeatletter
...
%% odkazy barevně
%% links in color
\hypersetup{colorlinks=true,linkcolor=glsfirstuse,urlcolor=glsfirstuse,citecolor=glsfirstuse}
%% odkazy černě
%% links in black
...
\definecolor{framecolour}{rgb}{0.85,0.85,0.85}

%%% Inline code with soft background: \inlinecode{identifier}
%%% Uses soul's \hl so long identifiers break across lines.
\usepackage{soul}
\definecolor{inlinecodebg}{HTML}{F2F2F2}
\definecolor{nohighlight}{HTML}{FFFFFF}
\sethlcolor{inlinecodebg}
\soulregister{\textbackslash}{0}
%%% soul's \hl paints background only across letter tokens. \_ as a command
%%% (or as an implicit \chardef character) leaves a gap in the highlight.
%%% Stash a cat-11 underscore token so \_ inside \inlinecode expands to
%%% something soul treats as a regular letter.
\begingroup
  \catcode`\_=11
  \gdef\inlinecodeus{_}
\endgroup
...
%%% Break-friendly monospace without background highlight: \brtt{identifier}
%%% Same breaking behaviour as \inlinecode but invisible (page-matching) highlight.
%%% Use inside tables where \texttt content would overflow narrow columns.
\DeclareRobustCommand{\brtt}[1]{%
  {\ttfamily\begingroup
    \sethlcolor{nohighlight}%
    \let\_\inlinecodeus
    \hl{#1}%
  \endgroup}%
}

%%% Minimal yaml language definition for listings package (not built in).
...
\lstset{
    ...
    basicstyle=\small\ttfamily\linespread{0.95}\selectfont,
    ...
    inputencoding=utf8,
    extendedchars=true,
    literate={—}{{\textemdash}}1 {–}{{\textendash}}1 {§}{{\S}}1 {→}{{$\rightarrow$}}1,
}

\lstnewenvironment{code}[3]{\lstset{language=#1, caption=#2, label=#3, numbers=left, xleftmargin=2em, frame=single, framexleftmargin=2em}}{}
\makeatletter
...
%%% English text definitions for thesis type and labels %%%
...
\def\TypPraceBP{BACHELOR'S THESIS}
\def\TypPraceDP{MASTER'S THESIS}
...
\ifstringequal{\TypPrace}{BP}{\def\TypPraceText{\TypPraceBP}\def\Praci{bachelor's thesis }}{}
\ifstringequal{\TypPrace}{DP}{\def\TypPraceText{\TypPraceDP}\def\Praci{master's thesis }}{}
